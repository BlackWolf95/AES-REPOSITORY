...
int main(){
  ...
  for(int round = 1; round<=9; round++){
    ...
    if(round == 1){
      ...
      mixcoln(initArr,round);
      uint8_t *temp3 = rearrange(initArr);//2 more lines to receive and swap them
      for(int i = 0;i < 16;i++)
        {
          initArr[i] = *(temp3 + i);
        }
    ...
    };//this is the first key word, to be XORed with initArr[]

    printf("\nKEY ROUND%d\n",round);
    print_array_uint8(key1,16);
    //rearrange(initArr);//TO MAKE THIS FUNCTION NOW!!!!!
    //XORing has to be differently done as well
      XORArr(initArr,key1,16);//final XORing upto 9th round
    /*initArr[0] =
    initArr[1] =
    initArr[2] =
    initArr[3] =
